...
class WorkflowConversionService:
    ...
    async def optimize_workflow(
        ...
        try:
            ...
            # For MVP: Assume workflow structure is available in context or via abstraction
            workflow_data = context.metadata.get(f"workflow_{workflow_id}", {})
            
            recommendations = []
            
            # Analyze workflow for coexistence opportunities
            # Identify tasks suitable for AI vs human
    ...
    async def generate_sop_from_structure(
        self,
        ...
        """
        Generate SOP from structured requirements (from agent).
        
        ARCHITECTURAL PRINCIPLE: Pure data processing - takes structure and generates SOP document.
        
        Args:
            sop_structure: SOP structure dict (title, description, steps, checkpoints, etc.)
            tenant_id: Tenant identifier
            context: Execution context
        ...
        """
        self.logger.info(f"Generating SOP from structure for tenant: {tenant_id}")
        
        try:
            sop_id = generate_event_id()
            title = sop_structure.get("title", "Standard Operating Procedure")
            ...
            expected_outcomes = sop_structure.get("expected_outcomes", [])
