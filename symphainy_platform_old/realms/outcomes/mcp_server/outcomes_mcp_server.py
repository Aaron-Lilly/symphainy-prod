...
Exposes Outcomes Orchestrator SOA APIs as MCP Tools for agent consumption.
"""
...
class OutcomesRealmMCPServer(MCPServerBase):
    """Outcomes Realm MCP Server."""
    
    def __init__(self, orchestrator):
        """Initialize Outcomes Realm MCP Server."""
        super().__init__(
            service_name="outcomes_mcp",
            realm_name="outcomes"
        )
    ...
    async def initialize(self) -> bool:
        ...
        try:
            self.logger.info("🔧 Initializing Outcomes Realm MCP Server...")
            ...
            for api_name, api_def in soa_apis.items():
                try:
                    ...
                    self.register_tool(
                        ...
                        description=api_def.get("description", f"Outcomes realm: {api_name}")
                    )
            ...
            self.logger.info(f"✅ Outcomes Realm MCP Server initialized with {registered_count} tools")
            return True
            
        except Exception as e:
            self.logger.error(f"❌ Failed to initialize Outcomes Realm MCP Server: {e}")
            return False
    
    def get_usage_guide(self) -> Dict[str, Any]:
        """Return usage guide."""
        return {
            "server_name": "outcomes_mcp",
            "realm": "outcomes",
            "description": "Outcomes Realm MCP Server",
