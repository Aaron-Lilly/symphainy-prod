...
2. **One MCP Server per Solution** - Prevents server sprawl
3. **Journeys compose intents** - Journey orchestrators compose intent services into complete user workflows
4. **compose_journey intent** - Special intent for programmatic journey invocation
...
The Solution class is the top-level organizational unit.
...
class ContentSolution:
    ...
    # Core methods
    async def handle_intent(intent, context) -> Dict  # Main entry point
...
- Implement handle_intent()
- Implement get_soa_apis()
- Implement get_experience_sdk_config()
