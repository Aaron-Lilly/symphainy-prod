...
from .services.structured_parsing_service.structured_parsing_service import StructuredParsingService
from .services.unstructured_parsing_service.unstructured_parsing_service import UnstructuredParsingService
from .services.hybrid_parsing_service.hybrid_parsing_service import HybridParsingService
from .services.workflow_sop_parsing_service.workflow_sop_parsing_service import WorkflowSOPParsingService
from .services.ingestion_service.ingestion_service import IngestionService
from .orchestrators.content_orchestrator import ContentOrchestrator
from .manager import ContentRealmManager
...
class ContentRealmFoundationService:
    ...
    def __init__(
        ...
        # Manager
        self.manager: Optional[ContentRealmManager] = None
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            # Initialize Content Orchestrator (needed for Ingestion Service)
            ...
            ingestion_abstraction = self.platform_gateway.get_ingestion_abstraction() if hasattr(self.platform_gateway, 'get_ingestion_abstraction') else None
            if ingestion_abstraction:
                ...
                self.logger.warning("Ingestion Service not created (ingestion abstraction missing)")
            
            # Initialize Content Realm Manager
            if self.curator:
                self.manager = ContentRealmManager(
                    curator=self.curator,
                    content_orchestrator=self.content_orchestrator
                )
                await self.manager.initialize()
                self.logger.info("✅ Content Realm Manager initialized")
            
            # Register services with Curator (legacy - now handled by Manager)
    ...
    def get_ingestion_service(self) -> Optional[IngestionService]:
