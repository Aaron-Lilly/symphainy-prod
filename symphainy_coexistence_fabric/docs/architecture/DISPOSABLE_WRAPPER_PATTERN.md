...
| `create_deterministic_embeddings()` | ⚠️ Review | Creates internal context object |

**Action:** Review `create_deterministic_embeddings()` — it creates a minimal `ExecutionContext` internally. This may be acceptable as "light shaping" or may need to be refactored to receive context from caller.
...
| `agents.get()` | ⚠️ Review | Lazy instantiation logic |
| `agents.invoke()` | ✅ Yes | Delegation to agent |
| `agents.collaborate()` | ⚠️ Review | Orchestration logic |

**Action:** Review lazy instantiation in `AgentService.get()` — it dynamically imports and creates agents. This is initialization logic, not business logic, so may be acceptable. The `collaborate()` method contains orchestration (calling multiple agents in sequence) which should potentially be moved to a higher layer.
