...
class TestControlTowerMCPServer:
    ...
    @pytest.mark.asyncio
    async def test_initialize_mcp_server(self, control_tower_solution):
        """Should initialize MCP server."""
        mcp_server = await control_tower_solution.initialize_mcp_server()
        assert mcp_server is not None
    
    @pytest.mark.asyncio
    async def test_mcp_server_has_tools(self, control_tower_solution):
