...
async def runtime_agent_websocket(
    ...
    # Require access_token for authentication (JWT from Supabase)
    # session_id is optional and used for session state
    # session_token is legacy parameter name, use access_token if provided
    auth_token = access_token or session_token
    if not auth_token:
        logger.warning("WebSocket connection rejected: No access token provided")
    ...
    if not app or not hasattr(app, 'state'):
        logger.error("WebSocket connection rejected: App state not available")
    ...
    try:
        ...
        if not hasattr(app.state, "security_guard_sdk"):
            logger.error("WebSocket connection rejected: Security Guard SDK not initialized")
        ...
        if not await connection_manager.connect(websocket, connection_id, user_id, tenant_id, ws_session_id):
            # Connection rejected (at capacity)
            return
        
        logger.info(f"Authenticated WebSocket connection: {connection_id} (user: {user_id}, tenant: {tenant_id}, session: {ws_session_id})")
        ...
        while True:
            try:
                ...
                conversation_id = context.get("conversation_id") or f"{surface}_{ws_session_id}"
