...
class GuideAgent(AgentBase):
    ...
    async def _process_with_assembled_prompt(
        ...
        ARCHITECTURAL PRINCIPLE: GuideAgent uses the assembled prompts and runtime context
        to provide platform navigation guidance. The system_message includes business context
        from runtime_context, and user_message contains the user's actual request.
        ...
        Returns:
            Dict with non-executing artifacts (guidance, recommendations):
            {
                ...
                "artifact": {
                    ...
                    "journey_guidance": Dict,
                    "routing_info": Optional[Dict]
                },
                "confidence": float
            }
        """
        # Extract message from user_message (may be the message itself or in a structured format)
        ...
        if hasattr(runtime_context, 'business_context') and runtime_context.business_context:
            # If message is in business_context, use it
        ...
        # If no message, use journey_goal as fallback
        if not message and runtime_context.journey_goal:
            message = runtime_context.journey_goal
        
        # If still no message, provide default guidance
        if not message:
            return {
                "artifact_type": "proposal",
                "artifact": {
                    "response": "I'm here to help you navigate the Symphainy Platform! What would you like to do?",
                    "intent_analysis": {
                        "intent_category": "navigation",
                        "confidence": 0.5
                    },
                    "journey_guidance": await self.get_journey_guidance(
                        user_state=await self._get_user_state(context.session_id, context.tenant_id, context),
                        context=context
                    ),
                    "routing_info": None
                },
        ...
        # Get session_id from request or context
        ...
        # Enhance with runtime context information if available
        if runtime_context.business_context:
            # Add business context to intent analysis if not already present
            intent_analysis = result.get("intent_analysis", {})
            if "business_context" not in intent_analysis:
                intent_analysis["business_context"] = runtime_context.business_context
        
        if runtime_context.journey_goal:
            # Add journey goal to journey guidance if not already present
            journey_guidance = result.get("journey_guidance", {})
            if "journey_goal" not in journey_guidance:
                journey_guidance["journey_goal"] = runtime_context.journey_goal
    ...
    async def process_request(
        ...
        # This allows orchestrators to provide runtime context (call site responsibility)
        if runtime_context:
