"""
Journey Solution - Journey Realm Solution

Composes journey orchestration for workflow management, SOP creation, and coexistence analysis.
"""
...
from .journeys.workflow_sop_journey import WorkflowSOPJourney
from .journeys.coexistence_analysis_journey import CoexistenceAnalysisJourney


class JourneySolution:
    """Journey Solution - Journey Realm Solution."""
    
    SOLUTION_ID = "journey_solution"
    SOLUTION_NAME = "Journey Solution"
    ...
    def _initialize_journeys(self):
        self._journeys["workflow_sop"] = WorkflowSOPJourney(public_works=self.public_works, state_surface=self.state_surface)
        self._journeys["coexistence_analysis"] = CoexistenceAnalysisJourney(public_works=self.public_works, state_surface=self.state_surface)
    ...
    async def initialize_mcp_server(self):
        if not self._mcp_server:
            from .mcp_server.journey_solution_mcp_server import JourneySolutionMCPServer
            self._mcp_server = JourneySolutionMCPServer(solution=self, public_works=self.public_works, state_surface=self.state_surface)
    ...
    def _find_journey_for_intent(self, intent_type: str) -> Optional[Any]:
        mapping = {
            "generate_sop": "workflow_sop",
            "create_workflow": "workflow_sop",
            "optimize_process": "workflow_sop",
            "analyze_coexistence": "coexistence_analysis",
            "identify_opportunities": "coexistence_analysis",
        }
