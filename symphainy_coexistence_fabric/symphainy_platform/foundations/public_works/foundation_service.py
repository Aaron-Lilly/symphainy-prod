...
from .abstractions.data_model_processing_abstraction import DataModelProcessingAbstraction
from .abstractions.workflow_processing_abstraction import WorkflowProcessingAbstraction
from .abstractions.sop_processing_abstraction import SopProcessingAbstraction
...
class PublicWorksFoundationService:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.file_management_abstraction: Optional[Any] = None  # FileManagementAbstraction
    ...
    async def _create_abstractions(self):
        ...
        # File management abstraction (REQUIRED for file operations like get_parsed_file)
        from .abstractions.file_management_abstraction import FileManagementAbstraction
        if self.gcs_adapter and self.supabase_file_adapter:
            ...
            self.logger.warning("File management abstraction not created (GCS or Supabase adapter missing)")
        ...
        # Data Model Processing Abstraction (uses JSON/YAML adapters)
        self.data_model_processing_abstraction = DataModelProcessingAbstraction(
            json_adapter=self.json_adapter,
            yaml_adapter=None,  # YAML adapter not yet implemented, uses built-in yaml library
            state_surface=temp_state_surface
        )
        
        # Workflow Processing Abstraction (BPMN, DrawIO)
        self.workflow_processing_abstraction = WorkflowProcessingAbstraction(
            workflow_adapter=None,  # Uses built-in BPMN parsing for now
            state_surface=temp_state_surface
        )
        
        # SOP Processing Abstraction (Markdown)
        self.sop_processing_abstraction = SopProcessingAbstraction(
            sop_adapter=None,  # Uses built-in Markdown parsing for now
            state_surface=temp_state_surface
        )
    ...
    def get_data_model_processing_abstraction(self) -> Optional[DataModelProcessingAbstraction]:
        """Get Data Model processing abstraction (for JSON Schema, YAML schemas)."""
        return self.data_model_processing_abstraction
    
    def get_workflow_processing_abstraction(self) -> Optional[WorkflowProcessingAbstraction]:
        """Get Workflow processing abstraction (for BPMN, DrawIO files)."""
        return self.workflow_processing_abstraction
