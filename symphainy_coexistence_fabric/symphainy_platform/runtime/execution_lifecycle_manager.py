...
class ExecutionLifecycleManager:
    ...
    async def execute(self, intent: Intent) -> ExecutionResult:
        ...
        try:
            ...
            # Phase 1: For ingest_file - request boundary contract from Data Steward (Civic Systems)
            if intent.intent_type == "ingest_file":
                ...
                try:
                    self.logger.info(f"🔍 Requesting boundary contract assignment from Data Steward for ingest_file intent")
                    ...
                    if not access_request.access_granted:
                        raise ValueError(f"Data access denied by Data Steward: {access_request.access_reason}")
                    
                    boundary_contract_id = access_request.contract_id
                    if not boundary_contract_id:
                        raise RuntimeError("Data Steward did not return boundary contract ID")
                    ...
                    # ✅ Fail if Data Steward is unavailable (don't create fallback - policy must be in Civic Systems)
    ...
    # Boundary contracts must be assigned by Civic Systems (Data Steward), not Runtime.
    # Policy decisions belong in Civic Systems, not Runtime Plane.
