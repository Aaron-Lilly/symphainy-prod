#!/usr/bin/env python3
"""
Test: XML File Parsing - Two-Phase Materialization Flow

Tests the parse_content capability for XML files using the two-phase flow.
"""
...
class TestXMLParsing(BaseCapabilityTest):
    def __init__(self):
        super().__init__(
            test_name="XML File Parsing - Two-Phase Materialization Flow",
            test_id_prefix="xml_parse"
        )
    ...
    async def run_test(self) -> bool:
        ...
        # Phase 1: Upload file
        self.print_info("Step 2: Phase 1 - Uploading XML file")
        xml_content = '<?xml version="1.0" encoding="UTF-8"?><users><user><name>John</name><age>30</age><city>New York</city></user><user><name>Jane</name><age>25</age><city>San Francisco</city></user></users>'
        test_file_name = f"test_parse_{datetime.now().strftime('%Y%m%d%H%M%S%f')}.xml"
        
        file_content_hex = xml_content.encode('utf-8').hex()
        ...
        self.print_info("Step 5: Validating parsed XML results")
        ...
        if "john" in content_str or "users" in content_str or "user" in content_str:
            self.print_success("Parsed XML contains expected data")
        else:
            self.print_warning("Parsed XML may not contain expected data")
        
        if isinstance(parsed_content, (list, dict)) and len(parsed_content) > 0:
            self.print_success("Parsed XML contains structured data")
        elif isinstance(parsed_content, str) and len(parsed_content) > 0:
            self.print_success(f"Parsed XML contains {len(parsed_content)} characters")
        else:
            self.print_error("Parsed XML is empty")
            return False
        
        self.print_success("✅ XML File Parsing - Two-Phase Materialization Flow: PASSED")
        return True
