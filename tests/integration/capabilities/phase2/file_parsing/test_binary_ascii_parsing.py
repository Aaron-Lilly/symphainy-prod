#!/usr/bin/env python3
"""
Test: Binary File Parsing (ASCII) - Two-Phase Materialization Flow

Tests the parse_content capability for ASCII-encoded binary files with copybook definitions.
"""
...
class TestBinaryASCIIParsing(BaseCapabilityTest):
    def __init__(self):
        super().__init__(
            test_name="Binary File Parsing (ASCII) - Two-Phase Materialization Flow",
            test_id_prefix="binary_ascii_parse"
        )
    ...
    async def run_test(self) -> bool:
        ...
        self.print_info("Step 2: Phase 1 - Uploading ASCII binary file")
        ...
        # Create test binary data (ASCII encoded)
        ...
        binary_record = (customer_id + customer_name + customer_age + customer_city).encode('ascii')
        ...
        binary_record2 = (customer_id2 + customer_name2 + customer_age2 + customer_city2).encode('ascii')
        
        binary_content = binary_record + binary_record2
        
        test_file_name = f"test_binary_ascii_{datetime.now().strftime('%Y%m%d%H%M%S%f')}.bin"
       ...
       01 CUSTOMER-RECORD.
        ...
        self.print_info("Step 5: Phase 3 - Parsing saved binary file with copybook")
        ...
        self.print_info("Step 6: Validating parsed binary (ASCII) results")
        ...
        self.print_success("✅ Binary File Parsing (ASCII) - Two-Phase Materialization Flow: PASSED")
        return True

async def main():
    test = TestBinaryASCIIParsing()
